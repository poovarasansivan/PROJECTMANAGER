...
$client->setClientId('853240356282-888am12j4ioudsfdku8esdln7a26s87m.apps.googleusercontent.com');
$client->setClientSecret('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');
$client->setRedirectUri('http://localhost/temp/auth/login/index.php');
...
if (isset($_GET['code'])) {
   ...
   if (!isset($token["error"])) {
      ...
      $get_user = mysqli_query($db, "SELECT * FROM user_login u INNER JOIN user_role r  ON u.role=r.id INNER JOIN m_resource m ON u.role=m.id INNER JOIN m_user us ON u.user_id=us.user_id  WHERE email='$email' AND u.status='1' and us.status ='1'");
      if (mysqli_num_rows($get_user) > 0) {
         ...
         header('Location: /temp/src/dashboard');
         exit;
                        ...
                        <div class="col mx-auto">
                           <div class="auth-form-box">
                              <div class="text-center mb-7">
                                 <a class="d-flex flex-center text-decoration-none mb-4" href="<?php echo $path ?>">
                              ...
                              <button id="btn" onclick="submitdata()" class="btn btn-primary w-100 mb-3">Sign
                                 In</button>
                                 <center><h4 style="padding-bottom: 2px;color:black; font-weight:90px">or</h4></center>
         ...
         $.ajax({
            ...
            success: function (result) {
               if (result.success) {
                  ...
                  window.location.href = window.location.href + '?error=' + encodeURIComponent(result.message);
               }
            },
            error: function (error) {
               console.log(error);
               window.location.href = window.location.href + '?error=Connection%20Error';
            }
         });
      }
   </script>
